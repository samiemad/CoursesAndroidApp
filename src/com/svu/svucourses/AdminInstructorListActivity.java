...
import com.svu.svucourses.adapter.AdminInstructorListAdapter;
...
public class AdminInstructorListActivity extends AppCompatActivity {

	RecyclerView rList;
	ProgressDialog progress;
	AdminInstructorListAdapter adapter;
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.mnu_instructors:
			...
			break;
		case R.id.mnu_new:
			startActivity(new Intent(this, AdminInstructorViewActivity.class));
			break;
		case R.id.mnu_search:
			startActivity(new Intent(this, AdminInstructorSearchActivity.class));
			break;
		}
		return true;
	}
	...
	private void loadCoursesList() {
		String url = App.BASE_URL + "getInstructors";
	...
	private Listener<String> lis = new Listener<String>() {
		...
		public void onResponse(String str) {
			progress.dismiss();
			try {
				ArrayList<Instructor> items = new ArrayList<Instructor>();
				JSONArray arr = new JSONArray(App.stripXml(str));
				for (int i = 0; i < arr.length(); ++i) {
					items.add(new Instructor((JSONObject) arr.opt(i)));
				}
				adapter = new AdminInstructorListAdapter(items);
